...
class UserAddress extends REST_Controller {
	...
	public function addUserAddress_post(){
		$requiredparameters = array('language','username','mobile','pincode','user_id','locality','fulladdress','state','city','addresstype','email','lat','lang');
		...
		$lang = removeSpecialCharacters($this->post('lang'));
    	...
    	if($validation=='valid') {
			if($username && is_numeric($mobile) && is_numeric($pincode) && $user_id  && $city && $addresstype){
				$address = $this->address_model->add_user_address($username,$mobile,$pincode,$user_id,$locality,$fulladdress,$state, $city, $addresstype,$email,$lat,$lang);
				if($address =='done'){					
