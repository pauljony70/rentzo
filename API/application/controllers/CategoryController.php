...
class CategoryController extends REST_Controller {
	...
	public function getCategory_post(){
    	...
    	if($validation=='valid') {
      		$category_array = $this->category_model->get_category_request($language_code,$devicetype);
	...
	public function getHomeCategory_post(){
    	...
    	if($validation=='valid') {
      		$category_array = $this->category_model->get_home_category_request($language_code,$devicetype);
	...
	public function getSubCategory_post(){
		$requiredparameters = array('language','catid','devicetype','pageno','config_attr');
    	...
    	if($validation=='valid') {
			if($catid || $catid  ==0){
				$category_array = $this->category_model->get_subcategory_request($language_code,$catid,$devicetype);
				...
				}else if(count($category_array) >0 || count($product_array) >0){	
	...
	public function getHeaderCategory_post(){
    	...
    	if($validation=='valid') {
      		$category_array = $this->category_model->get_header_category_request($language_code,$devicetype);
