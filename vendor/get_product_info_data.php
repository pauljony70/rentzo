...
if ($code == $_SESSION['_token']) {
    ...
    try {
        ...
        if (empty($attribute_set_id) && empty($product_id)) {
            $stmt = $conn->prepare("SELECT id,attribute,status FROM product_info_set ORDER BY created_at ASC LIMIT  " . $start . ", " . $limit . "");
            ...
            $stmt = $conn->prepare("SELECT product_info_set_id, attribute FROM attribute_set_product_info, product_info_set WHERE attribute_set_id = '" . $attribute_set_id . "' AND product_info_set.id = product_info_set_id ORDER By product_info_set.created_at ASC;");
            //$stmt->bind_param( s,  $inactive );
            $stmt->execute();
            $data = $stmt->bind_result($product_info_set_id, $attribute_name);
            $return = array();
            $i = 0;
            while ($stmt->fetch()) {
            ...
            foreach ($return as $key => $value) {
                $stmt18 = $conn->prepare("SELECT id, product_info_set_value FROM product_info_set_val WHERE product_info_set_id = '" . $value['product_info_set_id'] . "' ORDER By id ASC;");
                $stmt18->execute();
                $data = $stmt18->bind_result($product_info_set_value_id, $product_info_set_value);
