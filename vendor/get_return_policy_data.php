...
if($code == $_SESSION['_token']){
    ...
    try{
        ...
        $inactive = "active";
           $stmt = $conn->prepare("SELECT id,title,policy, status FROM product_return_policy where status ='1' ORDER BY title ASC LIMIT  ".$start.", ".$limit."");
		...
		$stmt12 = $conn->prepare("SELECT count(id) FROM product_return_policy  where status ='1' ");
