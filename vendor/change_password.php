...
if(isset($_POST['selleridvalue']) && isset($_POST['passwords']) ){
	...
	$passwords = $encruptfun->encrypt($publickey_server, $passwords);
   	...
   	$stmt11 = $conn->prepare("UPDATE sellerlogin SET password =? ,checksum ='' WHERE checksum ='".trim($_POST['selleridvalue'])."' ");
...
if (!isset($_GET['checksum']) || empty($_GET['checksum'])) {
	...
	$checksum = trim($_GET['checksum']);
	 $stmt = $conn->prepare("SELECT seller_unique_id,fullname,email,phone,status FROM sellerlogin WHERE checksum=?");
...
<meta name="keywords" content="Multi Vendor eCommerce app Seller panel, www.blueappsoftware.com" />
