...
if($code == $_SESSION['_token']){
    ...
    try{
        ...
        $inactive = "active";
           $stmt = $conn->prepare("SELECT brand_id, brand_name, brand_img,status,brand_name_ar FROM brand WHERE status ='1' ORDER BY brand_name ASC LIMIT  ".$start.", ".$limit."");
    	   //$stmt->bind_param( s,  $inactive );
    	   $stmt->execute();	 
     	   $data = $stmt->bind_result( $col1, $col2, $col3, $col4,$brand_name_ar);
           $return = array();
    	   $i =0;
       	   while ($stmt->fetch()) { 
    	       if( $col3){
		...
		$stmt12 = $conn->prepare("SELECT count(brand_id) FROM brand ");
