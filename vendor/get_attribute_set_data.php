...
if($code == $_SESSION['_token']){
    ...
    try{
        ...
        $inactive = "active";
           $stmt = $conn->prepare("SELECT sno, name, status FROM attribute_set where status ='1' ORDER BY name ASC LIMIT  ".$start.", ".$limit."");
		...
		$stmt12 = $conn->prepare("SELECT count(sno) FROM attribute_set where status ='1' ");
