...
$(document).ready(function () {
    ...
    $("#download_product").click(function (event) {
        ...
        location.href = "sample_configurable_import.csv";
    });
...
function uploadFile2(elementId, sizes) {
    if (!validateTempFile2(elementId, sizes)) return false;
    return true;
}
...
function uploadFile3(elementId) {
    if (!validateTempFile3(elementId)) return false;
    return true;
}

// Function to validate file
function validateTempFile3(elementId) {
    var filePath = jQuery('#' + elementId).val();
    if (jQuery.trim(filePath) == '') {
        successmsg('Please select Image file.')
        jQuery('#' + elementId).focus();
    ...
    if (!uploadFilter1(elementId, "csv", 'Please select CSV file.')) {
        return false;
    }
    return true;
}
...
function remove_notification() {
    ...
    $.ajax({
        ...
        url: 'pending_notification.php',
        data: {
            code: code_ajax, type: "remove_noti"
        },
        success: function (response) {
            ...
            if (data["status"] == "1") {
                ...
                $("#new_noti_count").text(data['count']);
                $("#new_noti_html").html(data['html_code']);
                successmsg("Notification Removed Successfully.");
            }
...
function download_last_import() {
    var code_ajax = $("#code_ajax").val();
    location.href = "download_last_import.php?code=" + code_ajax;
}

function undo_last_import() {
    var code_ajax = $("#code_ajax").val();
    location.href = "validate_product_process.php?type=undoimport&code=" + code_ajax;
}

function finsih_last_import() {
    var code_ajax = $("#code_ajax").val();
    location.href = "validate_product_process.php?types=finishimport&code=" + code_ajax;
}
