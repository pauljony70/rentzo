    ...
    try{
        ...
        $return = array();
           ...
           $stmt = $conn->prepare("SELECT pr.review_id, pr.rating, pr.title,pr.created_at,pr.status, pd.prod_name ,aul.fullname
			...
			FROM product_review pr, product_details pd , appuser_login aul,vendor_product vp WHERE aul.user_unique_id =pr.user_id AND pd.product_unique_id = pr.product_id and vp.product_id = pr.product_id and vp.vendor_id = '".$_SESSION['admin']."' AND   pr.status ='1' ORDER BY pr.created_at DESC LIMIT  ".$start.", ".$limit."");
		...
		$stmt12 = $conn->prepare("SELECT count(pr.review_id)
			...
			FROM product_review pr, product_details pd , appuser_login aul,vendor_product vp WHERE aul.user_unique_id =pr.user_id AND pd.product_unique_id = pr.product_id and vp.product_id = pr.product_id and vp.vendor_id = '".$_SESSION['admin']."' AND   pr.status ='1'  ");
