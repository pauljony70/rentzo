...
$order_id = $_POST['search_name_exp'];
$from_date = $_POST['from_date_exp'];
$to_date = $_POST['to_date_exp'];
...
$order_id =  stripslashes($order_id);
    ...
    try{
		...
		if($from_date != '' && $to_date){
			$order_id_qry .= ' and DATE(op.create_date) between "'.$from_date.'"  AND "'.$to_date.'"';
		}
		...
		header('Content-Type: text/csv; charset=utf-8');
			...
			fputcsv($output, array('Product Name', 'ProductID', 'Attributes', 'Quantity', 'MRP', 'Price', 'Shipping', 'Discount' , 'Order Status' , 'Date'));
			
			$exp = $conn->prepare("SELECT op.prod_id,op.prod_sku,op.prod_name,op.prod_img,op.prod_attr,op.qty,op.prod_price,op.shipping,op.discount,op.status,op.order_id ,op.create_date
            FROM  `order_product` op  WHERE vendor_id = '".$_SESSION['admin']."' ".$order_id_qry." ".$order_status_qry." ORDER BY op.id DESC");
			$exp->execute();	 
     	    $exp_data = $exp->bind_result( $col1, $col2, $col3, $col4, $col5, $col6, $col7, $col8, $col9, $col10, $col11, $col12);
			...
			while ($exp->fetch()) {   
				...
				$lineData = array($col3, $col1, $prod_attr, $col6, $col7+$col9, $col7, $col8, $col9, $order_status,$col12); 
          ...
          $stmt = $conn->prepare("SELECT op.prod_id,op.prod_sku,op.prod_name,op.prod_img,op.prod_attr,op.qty,op.prod_price,op.shipping,op.discount,op.status,op.order_id ,op.create_date
            FROM  `order_product` op  WHERE vendor_id = '".$_SESSION['admin']."' ".$order_id_qry." ".$order_status_qry." ORDER BY op.id DESC LIMIT ?, ?");
	   ...
	   $stmt ->bind_param("ii", $start, $limit);
          ...
          //echo " get col data ";
    	   while ($stmt->fetch()) {    
            ...
            $stmt12 = $conn->prepare("SELECT count(id) FROM order_product op WHERE vendor_id = '".$_SESSION['admin']."'   ".$order_id_qry." ".$order_status_qry."");
