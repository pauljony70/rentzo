...
// else if($code == $_SESSION['_token']){
    ...
    try{
		...
		if($from_date != '' && $to_date){
			...
			fputcsv($output, array('OrderID', 'Business Name', 'User Type', 'PaymentID', 'Amount', 'Quantity', 'Payment Mode', 'Order Date' , 'Payment Status' , 'Order Status'));
			
			$exp = $conn->prepare("SELECT o.order_id,o.user_id,o.status, o.total_price, o.payment_orderid,o.payment_id,o.payment_mode,o.qoute_id,o.create_date,
							o.discount,o.total_qty,op.vendor_id,sl.companyname FROM orders o join order_product op join sellerlogin sl WHERE op.order_id = o.order_id and sl.seller_unique_id = op.vendor_id  ".$order_id_qry."  ORDER BY o.sno DESC");
			$exp->execute();	 
     	    $exp_data = $exp->bind_result( $col1, $col2, $col3, $col4, $col5, $col6, $col7, $col8, $col9, $col10, $col11, $col12 , $col13 );
			...
			while ($exp->fetch()) {   
				...
				$lineData = array($col1, $col13, $user_type, $col6, $col4, $col11, $col7, $col9, 'Paid',$col3); 
			...
			$delimiter = ","; 
			$filename = "order-data_" . date('Y-m-d') . ".csv"; 
			 
			// Create a file pointer 
			$f = fopen('php://memory', 'w'); 
			 
			// Set column headers 
			$fields = array('OrderID', 'Business Name', 'User Type', 'PaymentID', 'Amount', 'Quantity', 'Payment Mode', 'Order Date' , 'Payment Status' , 'Order Status'); 
