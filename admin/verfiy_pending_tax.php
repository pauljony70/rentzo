...
if(!isset($_SESSION['admin'])){
 ...
 // echo " dashboard redirect to index";
   ...
   if(isset($deletearray) &&!empty( $deletearray)  ) {           
		...
		$stmt22 = $conn->prepare("SELECT name, created_by FROM  tax WHERE tax_id=? ");
		...
		$message = "Your Tax Class ".$name1." is rejected. ". $rejectreason;
		...
		$message = "Your Tax Class ".$name1." is verified.";
		$stmt11 = $conn->prepare("UPDATE  tax SET status =1 WHERE tax_id = '".trim($_POST['verify_record_id'])."'");
