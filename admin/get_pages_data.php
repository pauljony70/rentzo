...
if($code == $_SESSION['_token']){
    ...
    try{
        ...
        $inactive = "active";
           $stmt = $conn->prepare("SELECT pages_id,page_title,page_dsc FROM pages  ORDER BY page_title ASC LIMIT  ".$start.", ".$limit."");
		...
		$stmt12 = $conn->prepare("SELECT count(pages_id) FROM pages ");
