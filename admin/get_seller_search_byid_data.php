...
if(!isset($_SESSION['admin'])){
  header("Location: index.php");
    ...
    try{
        ...
        $search = $seller_id;
         if($groupid!= "blank"){
             $stmt12 = $conn->prepare("SELECT count(sellerid) FROM sellerlogin WHERE groupid =? AND sellerid=?");
             $stmt12 ->bind_param(ii, $groupid, $search);
            
         }else{
            $stmt12 = $conn->prepare("SELECT count(sellerid) FROM sellerlogin WHERE sellerid=?");
             $stmt12 ->bind_param(i, $search);        
         }
        $stmt12->execute();
        $stmt12->store_result();
        ...
        $i      = 0;
            ...
            if($groupid!= "blank"){
                 ...
                 $stmt = $conn->prepare("SELECT sl.sellerid, sl.companyname, sl.fullname, sl.email, sl.phone, ct.city_name, sg.name, sl.create_by, sl.tax_number, sl.status FROM sellerlogin sl, seller_group sg, city ct
                    WHERE sl.groupid = sg.sno AND sl.city = ct.city_id AND sl.groupid=? AND sl.sellerid= ? ORDER BY sl.create_by DESC LIMIT ?, ?");
                   $stmt ->bind_param(iiii,$groupid,$search, $start, $limit);
            	
            }else{
       
                $stmt = $conn->prepare("SELECT sl.sellerid, sl.companyname, sl.fullname, sl.email, sl.phone, ct.city_name, sg.name, sl.create_by, sl.tax_number, sl.status FROM sellerlogin sl, seller_group sg, city ct
                WHERE sl.groupid = sg.sno AND sl.city = ct.city_id AND sl.sellerid= ? ORDER BY sl.create_by DESC LIMIT ?, ?");
               $stmt ->bind_param(iii,$search, $start, $limit);
            }
    	   $stmt->execute();	 
