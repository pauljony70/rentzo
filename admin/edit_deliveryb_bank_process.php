...
&& !empty($accno)  && !empty($ifsc)  && !empty($address)  ) {
       ...
       // get from db-connection 	date_default_timezone_set("Asia/Kolkata");
        ...
        $status = 0;
   	    ...
   	    $stmt11 = $conn->prepare("UPDATE deliveryboy_bankdetails SET bank_name=?, acc_no=?, ifsccode=?, address=?, createby=?, updateby=?, verifed=?, verified_date=? WHERE deliveryboy_id=?");
		$stmt11->bind_param( "ssssssiss", $bankname, $accno, $ifsc, $address, $datetime, $datetime, $verify, $verifydate, $deliveryboy_id  );
		 $stmt11->execute();
    	 $stmt11->store_result();
