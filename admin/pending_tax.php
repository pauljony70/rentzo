...
<script >
    ...
    function deleteRecord(cat_id){
		...
		if(!rejectreason){
		 ...
		 xdialog.confirm('Are you sure want to reject Tax Class?', function() {
          showloader();
            $.ajax({
                  method: 'POST',
                  url: 'verfiy_pending_tax.php',
                  ...
                  success: function(response){
					...
					if(response =='Failed to Delete.'){
						...
						$("#tr"+cat_id).remove();
						successmsg("Tax Class Rejected Successfully.");
	...
	function verifyRecord(cat_id){
		 ...
		 xdialog.confirm('Are you sure want to approve Tax Class?', function() {
          showloader();
            $.ajax({
                  method: 'POST',
                  url: 'verfiy_pending_tax.php',
                  ...
                  success: function(response){
					...
					if(response =='Failed to approve.'){
						...
						$("#tr"+cat_id).remove();
						successmsg("Tax Class approved successfully.");
...
<?php 
	 // echo "class id is  ".$class_id;     
        $inactive = "active";
        $parentname ="";
           $stmt = $conn->prepare("SELECT tax_id , name,percent,status FROM tax  WHERE status IN(0) ORDER BY name ASC");
