...
if(!isset($_SESSION['admin'])){
 ...
 // echo " dashboard redirect to index";
   ...
   if(isset($deletearray) &&!empty( $deletearray)  ) {           
		...
		$stmt22 = $conn->prepare("SELECT attribute, created_by FROM  product_attributes_set WHERE id=? ");
		...
		$stmt11 = $conn->prepare("UPDATE  product_attributes_set SET status =1 WHERE id = '".trim($_POST['verify_record_id'])."'");
