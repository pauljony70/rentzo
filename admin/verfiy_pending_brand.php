...
if(!isset($_SESSION['admin'])){
 ...
 // echo " dashboard redirect to index";
   ...
   if(isset($deletearray) &&!empty( $deletearray)  ) {           
		...
		$stmt22 = $conn->prepare("SELECT brand_name, created_by FROM  brand WHERE brand_id=? ");
		$stmt22->bind_param( "i",  $deletearray );
		$stmt22->execute();	 
		$data = $stmt22->bind_result( $brand_name, $created_by);
		$return = array();
		...
		$message = "Your Brand ".$brand_name1." is rejected. ". $rejectreason;
		...
		$message = "Your Brand ".$brand_name1." is verified.";
		$stmt11 = $conn->prepare("UPDATE  brand SET status =1 WHERE brand_id = '".trim($_POST['verify_record_id'])."'");
