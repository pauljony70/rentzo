...
if (!isset($_SESSION['admin'])) {
   ...
   try{
		...
		if(isset($_POST['prod_name'])){
			...
			$query = "SELECT pd.product_unique_id, pd.prod_name, pd.featured_img,  pd.status ,brand.brand_name,pd.product_sku  FROM product_details pd,brand WHERE brand.brand_id  = pd.brand_id AND (pd.product_unique_id like '%".$search_text."%'  OR pd.prod_name like '%".$search_text."%'  OR pd.product_sku like '%".$search_text."%'  )
				AND pd.status NOT IN(1,3)  ".$order_by." LIMIT ?, ?";
			
			$query_total = "SELECT count(pd.product_unique_id) FROM product_details pd,brand WHERE brand.brand_id  = pd.brand_id AND (pd.product_unique_id like '%".$search_text."%'  OR pd.prod_name like '%".$search_text."%'  OR pd.product_sku like '%".$search_text."%'  ) AND pd.status NOT IN(1,3) ";
		}else{
			$query = "SELECT pd.product_unique_id, pd.prod_name, pd.featured_img,  pd.status ,brand.brand_name,pd.product_sku
				FROM product_details pd,brand WHERE brand.brand_id  = pd.brand_id AND pd.status NOT IN(1,3)   ".$order_by." LIMIT ?, ?";
				
			$query_total = "SELECT count(pd.product_unique_id) FROM product_details pd,brand WHERE brand.brand_id  = pd.brand_id AND pd.status NOT IN(1,3)  ";
		}
		...
		foreach($product_arr_final as $products){
            ...
            $tbl_html .=  '<td>Pending</td>';
            $tbl_html .=  '<td>
								<button style=" margin-left: 10px;font-size: 12px;" type="submit" class= "btn btn-dark waves-effect waves-light btn-sm pull-left" name="view" onclick=viewRecord("'.$products['product_unique_id'].'");>View</button>
