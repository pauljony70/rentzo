...
if($code == $_SESSION['_token']){
    ...
    try{
        ...
        $inactive = "active";
           $stmt = $conn->prepare("SELECT id, noti_title, noti_body FROM firebase_notification  ORDER BY id LIMIT  ".$start.", ".$limit."");
    	   ...
    	   $i =0;
       	   while ($stmt->fetch()) { 
        	   	...
        	   	$return[$i] = 
        					array(	
        					    'id' => $col1,
        						'noti_title' => $col2,'noti_body' => $col3);
              		   $i = $i+1;  	
              	$status = 1;
                $msg = "Details here";
		...
		$stmt12 = $conn->prepare("SELECT count(id) FROM firebase_notification");
