...
if ($code == $_SESSION['_token']) {
	...
	try {
		...
		$inactive = "active";
		$stmt = $conn->prepare("SELECT brand_id, brand_name, brand_img,status,brand_name_ar,brand_site_url,popular_brand FROM brand WHERE  status IN(1,3) ORDER BY brand_name ASC LIMIT  " . $start . ", " . $limit . "");
		//$stmt->bind_param( s,  $inactive );
		$stmt->execute();
		$data = $stmt->bind_result($col1, $col2, $col3, $col4, $brand_name_ar, $brand_site_url, $popular_brand);
		$return = array();
		$i = 0;
		while ($stmt->fetch()) {
			if ($col3) {
		...
		$stmt12 = $conn->prepare("SELECT count(brand_id) FROM brand WHERE status IN(1,3) ");
