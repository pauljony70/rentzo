...
if($code == $_SESSION['_token']){
    ...
    try{
        ...
        $inactive = "active";
           $stmt = $conn->prepare("SELECT id,title, description, status FROM faq WHERE status IN(1,3)  ORDER BY id ASC LIMIT  ".$start.", ".$limit."");
		...
		$stmt12 = $conn->prepare("SELECT count(id) FROM faq WHERE status IN(1,3)  ");
