...
if($code == $_SESSION['_token']){
    ...
    try{
        ...
        $inactive = "active";
           ...
           $stmt = $conn->prepare("SELECT id,hsn_code,status FROM product_hsn_code WHERE status ='1' ORDER BY id ASC LIMIT  ".$start.", ".$limit."");
		...
		$stmt12 = $conn->prepare("SELECT count(id) FROM product_hsn_code ");
		...
		$page_html =  $Common_Function->pagination('product_hsn_code',$page,$limit,$totalrow); 
