...
if(!isset($_SESSION['admin'])){
 ...
 // echo " dashboard redirect to index";
}else if($code == $_SESSION['_token'] && isset($sellerid)  && !empty($sellerid) ){
    ...
    //    $decript2 = $encruptfun->decrypt($publickey_server, $encrypted1);
        $stmt = $conn->prepare("SELECT bank_name, acc_no, ifsccode, address, createby, 	updateby, verifed, verified_date FROM seller_bankdetails WHERE seller_id=?");
