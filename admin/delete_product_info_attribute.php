...
if (!isset($_SESSION['admin'])) {
    ...
    if (isset($deletearray) && !empty($deletearray)) {
        ...
        $stmt = $conn->prepare("SELECT id FROM product_info WHERE product_info_set_id=? ");
        ...
        if ($exist == "Y") {
            echo "This Attribute already assign to some products.You can't delete the attribute as it is assigned to a product. Please delete product first";
        } else {
            $Common_Function->delete_product_info_attribute($deletearray, $conn);
        }
        die;
    }
}
