...
if($code == $_SESSION['_token']){
    ...
    try{
        ...
        $inactive = "active";
           $stmt = $conn->prepare("SELECT tax_id,name, percent, status FROM tax WHERE status IN(1,3)  ORDER BY percent ASC LIMIT  ".$start.", ".$limit."");
		...
		$stmt12 = $conn->prepare("SELECT count(tax_id) FROM tax WHERE status IN(1,3)  ");
