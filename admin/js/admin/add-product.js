...
function add_more_attrs() {
    ...
    if (sel_attr != '1') {
        ...
        attr_html += '<div id="cselectattrs' + i + '" style="display:flex; flex-wrap:wrap;"></div><button type="submit" class="btn btn-sm btn-danger" onclick=removeattrs("selectattrs_div' + i + '"); return false;" style="float:left; display: inline-block; margin-right:20px;">Remove</button>';
...
function check_product() {
    ...
    if (!prod_name) {
        ...
        var html = '<div id="cselectattrs" style="display:flex; flex-wrap:wrap;"></div></div><br>  </div> </div>   </div>';
...
$(document).ready(function () {
    $("#addProduct_btn").click(function (event) {
        ...
        var selectattrset = $('#selectattrset');
        var prod_namevalue = $('#prod_name');
        var prod_shortvalue = tinyMCE.get('prod_short');
        var prod_detailsvalue = tinyMCE.get('editor');
        var prod_mrpvalue = $('#prod_mrp');
        var prod_pricevalue = $('#prod_price');
        var prod_brand = $('#selectbrand');
        var prod_seller = $('#selectseller');
        var featured_img = $('#featured_img');
        var prod_name_ar = $('#prod_name_ar');
        var prod_short_ar = tinyMCE.get('prod_short_ar');
        var editor_ar = tinyMCE.get('editor_ar');

        /* if (prod_brand == '') {
            prod_brand = $('#selectbrand').val('3');
        } */

        /*var mrp = $('#prod_mrp').val(prod_pricevalue);*/
        ...
        if (!selectattrset.val()) {
            ...
            prod_mrpvalue.focus();
            valid = 0;
        } else if (!prod_pricevalue.val()) {
            successmsg("Price is empty");
            $('#seller_price').focus();
            valid = 0;
        } else if (parseFloat(prod_mrpvalue.val()) < parseFloat(prod_pricevalue.val())) {
            successmsg("Please enter product Price less or equal to MRP");
            prod_mrpvalue.focus();
            valid = 0;
        } else if (prod_brand.val() == "") {
