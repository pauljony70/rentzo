        ...
        success: function (response) {
            ...
            if (data["status"] == "1") {
                ...
                $(data["details"]).each(function () {
                    ...
                    public_html = '<tr> <td>' + count + '</td><td> ' + this.companyname + '</td><td class="nr">' + this.amount + '</td><td>' + this.date_range + '</td> <td>' + this.payment_status + '</td> <td>' + this.transection_id + '</td><td><div class="d-flex">';

                    if (this.payment_status == 'pending') {
                        public_html += '<button type="submit"  class= "btn btn-danger waves-effect waves-light btn-sm pull-left open-modal" target="_blank" data-pay_id="' + this.id + '" data-id="' + this.seller_id + '" data-toggle="modal" data-target="#myModal">Pay</button>';

                        public_html += '<a style="margin-left:10px;" class= "btn btn-dark waves-effect waves-light btn-sm pull-left" target="_blank" href="manage_order_date_wise_transaction.php?id=' + this.id + '">View All</a></div></td></tr> ';
                        $("#tbodyPostid").append(public_html);
                    } else {

                        public_html += '<a class= "btn btn-dark waves-effect waves-light btn-sm pull-left" target="_blank" href="manage_order_date_wise_transaction.php?id=' + this.id + '">View All</a></div></td></tr> ';
                        $("#tbodyPostid").append(public_html);
                    }
