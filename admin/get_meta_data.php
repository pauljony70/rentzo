...
if($code == $_SESSION['_token']){
    ...
    try{
        ...
        $inactive = "active";
           $stmt = $conn->prepare("SELECT metaid, page_title, page_heading,meta_tags,meta_desc,meta_keys,page_content,caninocial_url FROM seometa  ORDER BY page_title ASC LIMIT  ".$start.", ".$limit."");
		...
		$stmt12 = $conn->prepare("SELECT count(metaid) FROM seometa ");
