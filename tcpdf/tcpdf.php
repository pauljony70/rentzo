...
class TCPDF {
	...
	/**
	 * Set the watermark path.
	 * @param $watermark (path) watermark imagefull path
	 * @public
	...
	public function Header() {
		if($this->getWatermark()){
			...
			// set bacground image
			$img_file = K_PATH_IMAGES.$this->getWatermark();
			$this->StartTransform();
			$this->Rotate(45,150, 100);
			$this->Image($img_file, 50, 50, 80, 57, '', '', '', false, 100, '', false, false, 0);
			$this->StopTransform();
	...
	protected function _putcatalog() {
		...
		if (!empty($this->efnames)) {
			...
			/*foreach ($this->efnames AS $fn => $fref) { //code comment by sandeep for remove duplicate file name
