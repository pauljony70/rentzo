...
class Email_model extends CI_Model
{
	...
	function sendEmail_faq($ticket_id,$name, $email, $subject, $content)
	{
		$system_name = get_settings('system_name');
	...
	function sendEmail_ticket($ticket_id,$subject, $content)
	{
		
		$user_data = $this->db->get_where('faq_form', array('ticket_id' => $ticket_id))->row_array();
		$email = $user_data['user_email'];
		
		
		$system_name = get_settings('system_name');
		$subjects = $system_name . ' Your Content Inquiry Details Replay';
		$email_body =
			'<div style="font-family: Helvetica,Arial,sans-serif;min-width:1000px;overflow:auto;line-height:2">
				<div style="margin:50px auto;width:70%;padding:20px 0">
					<div style="border-bottom:1px solid #eee">
