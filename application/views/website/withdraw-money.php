                                ...
                                <div class="card-body">
                                    <div class="row p-0">
                                        ...
                                        <div class="col-6 p-0 <?= $default_language == 1 ? "text-start" : "text-end" ?>">
                                            <a class="btn money-btn px-3 py-1" href="<?= base_url ?>my-wallet">
                                                <div class="d-flex justify-content-center align-items-center h-100">
                                                    <i class="fa-solid fa-circle-plus"></i>
                                                    <div class="mx-2 fw-bolder text-uppercase" style="margin-top: 0.15rem;"><?= $this->lang->line('wallet-transfer-money') ?></div>
                                                </div>
                                            </a>
                                        </div>
                                    </div>
                                    ...
                                    <p class="card-text"><?= $this->lang->line('wallet-withdraw-desc') ?></p>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <form action="">
                                        ...
                                        <button type="button" class="btn btn-primary btn-lg text-uppercase w-100 my-5" id="withdraw-money-btn">
                                            <div class="d-flex justify-content-center align-items-center h-100">
                                                <?= $this->lang->line('wallet-withdraw-money') ?>
                                            </div>
                                        </button>
                                    </form>
                                </div>
    ...
    <script>
        ...
        function addMoneyToWalletField(amount) {
            ...
            if (curr_val == "") {
                ...
                let t_money = parseFloat(curr_val) + parseFloat(amount);
                sel.value = t_money;
            }
        }

        document.querySelector('#withdraw-money-btn').addEventListener('click', (event) => {
            buttonLoader(document.querySelector('#withdraw-money-btn').querySelector('.d-flex'));
            document.querySelector('#withdraw-money-btn').disabled = true;
            if (sel.value > 0) {
                $.ajax({
                    method: "post",
                    url: site_url + "send-withdraw-money-request",
                    data: {
                    ...
                    success: function(response) {
                        document.querySelector('#withdraw-money-btn').querySelector('.d-flex').innerHTML = '<?= $this->lang->line('wallet-withdraw-money') ?>';
                        document.querySelector('#withdraw-money-btn').disabled = false;
                        if (response.status) {
                            Swal.fire({
                                title: 'SUCCESS',
                                text: response.msg,
                            ...
                            setErrorMsg(sel, '<i class="fa-solid fa-circle-xmark"></i> ' + response.msg);
                        }
                    },
