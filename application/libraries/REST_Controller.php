...
abstract class REST_Controller extends CI_Controller
{
    ...
    public function __construct($config = 'rest')
    {
        ...
        $system_timezone = $this->db->get_where('settings', array(
        ...
        $this->default_language = 2;
        // Set the language folder based on the session data
        $languageCode = $this->session->userdata('default_language');
        $languageFolder = ($languageCode == 1) ? 'arabic' : 'english';
        $this->lang->load($languageFolder, $languageFolder);
    ...
    {
        if ($this->request_method != $method = $this->input->method()) {
            $this->response([
                $this->config->item('rest_status_field_name') => 0,
                $this->config->item('rest_message_field_name') => "Invalid Request."
            ], self::HTTP_FORBIDDEN);
        ...
        if ($this->config->item('rest_enable_keys') && $use_key && $this->_allow === FALSE) {
            ...
            $this->response([
                $this->config->item('rest_status_field_name') => 0,
                $this->config->item('rest_message_field_name') => sprintf('Invalid Request', $this->rest->key)
            ], self::HTTP_FORBIDDEN);
            $this->is_valid_request = false;
        }
    ...
    protected function parameterValidation($required, $postvalues)
    {
        $missing = array();
        ...
        if (count($missing) >= 1) {
            ...
            $msg =  implode(", ", $missing) . ' ' . $count . ' missing parameter in request';
    ...
    protected function responses($status, $message, $data = [])
    {
        // Display an error response
        $this->response([
